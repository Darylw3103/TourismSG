...
export default function Hotels() {
  ...
  // Fetch details when a hotel is selected
  useEffect(() => {
    if (selectedHotel) {
  ...
  return (
    <div className="min-h-screen bg-blue-200 flex">
      ...
      {/* Left-side fixed hotel image */}
      <div className="fixed top-0 left-0 h-full w-1/3">
        <img
          src={HotelPic}
          alt="Hotel"
                  ...
                  <div className="space-y-4">
                    {selectedHotelDetails.price_range && (
                      <div>
                        ...
                        <p className="text-green-600 font-semibold text-lg">
                          {selectedHotelDetails.price_range.toString().includes("-")
                            ? selectedHotelDetails.price_range
                                .toString()
                                .replace(/(\d+)\s*-\s*(\d+)/, "$$$1 - $$$2")
                            : `$${selectedHotelDetails.price_range}`}
                    ...
                    {selectedHotelDetails.contact_info && (
                      <div>
                        ...
                        <p className="text-gray-900">{selectedHotelDetails.contact_info}</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {hotels.map((hotel, index) => (
                ...
                >
                  ...
                  {hotel.price_range && (
                    <p className="text-green-600 font-semibold">{hotel.price_range}</p>
                  )}
                  <button
                    onClick={() => {
